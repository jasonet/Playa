...
archive_path=""
derived_data_path="${PLAYA_DERIVED_DATA:-build/XcodeDerivedData}"
...
if [[ -z "$archive_path" ]]; then
    archive_path="dist/archive/Playa-$(date +%Y%m%d-%H%M%S).xcarchive"
fi
...
cd "$repository_root"
if [[ "$generate_project" == true ]]; then
    echo "Generating Playa.xcodeproj..."
    xcodegen generate
fi
...
[[ ! -e "$archive_path/Products/Library/Frameworks/PlayaServerKit.framework" ]] || \
    fail "PlayaServerKit was installed as a top-level archive product; check SKIP_INSTALL"
