...
const productScreens = {
  chat: {
    src: 'assets/screenshots/playa-main.png',
    alt: 'Playa chat running the Gemma 4 model locally, with response token and memory telemetry',
    label: '01 / CHAT',
    caption: 'Talk to open models with streaming responses and per-message performance metrics.'
  },
  dashboard: {
    src: 'assets/screenshots/playa-dashboard.png',
    alt: 'Playa analytics dashboard showing local token usage, requests, success rate, and decode speed',
    label: '02 / ANALYTICS',
    caption: 'See token usage, request volume, success rate, and decode speed across your local workspace.'
  },
  models: {
    src: 'assets/screenshots/playa-models.png',
    alt: 'Playa model library with the CohereLabs North-Mini-Code model selected',
    label: '03 / MODELS',
    caption: 'Manage installed models and understand context, size, modalities, and tool support at a glance.'
  },
  integrations: {
    src: 'assets/screenshots/playa-integrations.png',
    alt: 'Playa integrations screen showing Pi, Codex, Claude Code, Hermes, and OpenCode configured to use local models',
...
fetch('https://api.github.com/repos/jasonet/Playa/releases/latest', {
  cache: 'no-store',
...
fetch('https://api.github.com/repos/jasonet/Playa')
  .then((response) => (response.ok ? response.json() : Promise.reject()))
  .then((repository) => {
