...
OVERLAY_SERVER = PYTHON_DISTRIBUTION_ROOT / "Overlay" / "playa_server.py"
...
def fetch_json(url: str) -> dict:
    request = urllib.request.Request(url, headers={"User-Agent": "playa-builder"})
...
PARENT_PID="$PPID"
LAUNCHER_PID="$$"
"$ROOT_DIR/python/bin/python3" -m playa_server "$@" &
CHILD_PID="$!"
