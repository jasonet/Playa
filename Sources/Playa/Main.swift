...
enum Main {
    static func main() {
        if CommandLine.arguments.contains("--fx-image-marker-smoke-test") {
            do {
                print(try FxAgentHarness.runImageMarkerSmokeTest())
                exit(EXIT_SUCCESS)
            } catch {
                fputs("\(error)\n", stderr)
                exit(EXIT_FAILURE)
        ...
        if CommandLine.arguments.contains("--smoke-test") {
            do {
                let output = try Playa.run(arguments: ["--help"])
                print(output)
                exit(EXIT_SUCCESS)
            } catch {
                fputs("\(error)\n", stderr)
        ...
        if CommandLine.arguments.contains("--lifecycle-smoke-test") {
            let server = PlayaProcessController()
            ...
            do {
                let smokePort = ProcessInfo.processInfo.environment["PLAYA_SMOKE_PORT"] ?? "18080"
        ...
        .commands {
            ...
            CommandMenu("Conversation") {
                Button("Next Conversation") {
                    appDelegate.selectNextChat()
                }
                .keyboardShortcut(.tab, modifiers: [.control])

                Button("Previous Conversation") {
                    appDelegate.selectPreviousChat()
                }
                ...
                Button("Increase Text Size") {
                    appDelegate.adjustChatTextSize(by: 1)
                }
...
private struct PlayaRootView: View {
