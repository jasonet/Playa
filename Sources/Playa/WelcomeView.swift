    ...
    private var welcomeHeader: some View {
        VStack(spacing: 16) {
            Image(nsImage: NSApplication.shared.applicationIconImage)
                ...
                .accessibilityLabel("Playa app icon")
                    ...
                    HStack(alignment: .top, spacing: 14) {
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text("Protect management access")
                                .font(.headline)
                            Text("An API key provides basic security if you're running Playa on a shared network.")
                                .font(.callout)
                                .foregroundStyle(.secondary)
...
private enum WelcomeAPIKeyGenerator {
    static func makeKey() -> String {
        ...
        guard SecRandomCopyBytes(kSecRandomDefault, bytes.count, &bytes) == errSecSuccess else {
            return "playa_\(UUID().uuidString.replacingOccurrences(of: "-", with: "").lowercased())"
        }
...
#Preview {
    WelcomeView(model: PlayaModel()) { _, _ in }
