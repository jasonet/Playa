...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    private var textSizeShortcutMonitor: Any?

    func applicationDidFinishLaunching(_ notification: Notification) {
        applyApplicationIcon()
        installTextSizeShortcutMonitor()
        runtime.start()
    ...
    func applicationWillTerminate(_ notification: Notification) {
        if let textSizeShortcutMonitor {
            NSEvent.removeMonitor(textSizeShortcutMonitor)
            self.textSizeShortcutMonitor = nil
        }
    ...
    @objc private func switchModelFromMenu(_ sender: NSMenuItem) {
        ...
        model.selectModel(to: rawModelID?.isEmpty == false ? rawModelID : nil)
    }
    ...
    func selectNextChat() {
        controlPanelNavigation.navigateChatSession(.next)
        showMainWindow()
    }

    func selectPreviousChat() {
        controlPanelNavigation.navigateChatSession(.previous)
        showMainWindow()
    }

    func adjustChatTextSize(by pointDelta: Double) {
        var settings = model.settings
        settings.chatBodyFontSize += pointDelta
        model.settings = settings.normalized()
    }

    private func installTextSizeShortcutMonitor() {
        guard textSizeShortcutMonitor == nil else {
            return
        }

        textSizeShortcutMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) {
            [weak self] event in
            guard let self,
                  let pointDelta = self.textSizeShortcutDelta(for: event)
            else {
                return event
            }

            self.adjustChatTextSize(by: pointDelta)
            return nil
        }
    }

    private func textSizeShortcutDelta(for event: NSEvent) -> Double? {
        let modifiers = event.modifierFlags.intersection([.command, .control, .option, .shift])

        // Equal and plus share the same physical key on a standard Mac keyboard.
        if event.keyCode == 24,
    ...
    private func rebuildMenu() {
        ...
        if model.sessionStatsDisplayMetrics != nil || model.isRunning || sessionStatsAreLoading {
            ...
            let statusMenuItem = NSMenuItem(
                title: model.isRunning ? model.unavailableMetricsText : "Playa Server is Not Running",
                action: nil,
                keyEquivalent: ""
            )
    ...
    private func modelRowMenuItem(_ localModel: LocalModel) -> NSMenuItem {
        ...
        item.view = ModelMenuRowView(
            ...
            onSelect: { [weak self] in
                self?.model.selectModel(to: localModel.repoID)
            }
        )
        return item
    }
    ...
    private func modelDisplayName(_ modelID: String) -> String {
        let shortName = modelID.split(separator: "/").last.map(String.init) ?? modelID
        return PlayaFormatting.truncateModelName(shortName, maxLength: 34)
    }

    private func missingModelMenuLabel(_ modelID: String) -> String {
        let shortName = modelID.split(separator: "/").last.map(String.init) ?? modelID
        return "\(PlayaFormatting.truncateModelName(shortName, maxLength: 34))  ·  Not found"
    }
    ...
    private func makeServingStatsSubmenu() -> NSMenu {
        ...
        addSection("Session", entries: PlayaStats.sessionEntries(metrics), to: submenu)

        submenu.addItem(.separator())
        addSection("All-Time", entries: PlayaStats.allTimeEntries(model.allTimeStats), to: submenu)

        if let latest = metrics.latest {
            submenu.addItem(.separator())
            addSection("Latest Request", entries: PlayaStats.latestRequestEntries(latest), to: submenu)
        }

        submenu.addItem(.separator())
        addSection("Runtime", entries: PlayaStats.runtimeEntries(metrics.server), to: submenu)
...
private struct SessionStatsMenuView: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel(section == .header
            ? "Playa Server status"
            : "Playa Server session statistics")
    }
    ...
    private var header: some View {
        HStack(alignment: .top) {
            ...
            VStack(alignment: .trailing, spacing: 1) {
                ...
                Text(PlayaFormatting.truncateModelName(
                    displayModel,
                    maxLength: 20
                ))
                .font(.subheadline)
    ...
    private var sessionOverview: some View {
        VStack(alignment: .leading, spacing: 7) {
            HStack(alignment: .firstTextBaseline) {
                ...
                metric(
                    "Average decode",
                    PlayaFormatting.rate(metrics.summary.averageDecodeTokensPerSecond),
                    alignment: .trailing
                )
    ...
    private var metricsGrid: some View {
        LazyVGrid(
            ...
            metric("In flight", PlayaFormatting.integer(metrics.summary.inFlight))
            metric("Uptime", PlayaFormatting.duration(metrics.summary.uptimeSeconds))
        }
    }
    ...
    private func latestRequest(_ latest: PlayaLatestRequest) -> some View {
        HStack(alignment: .firstTextBaseline) {
            metric(
                "Latest request",
            ...
            metric(
                "Prefill speed",
                PlayaFormatting.rate(latest.prefillTokensPerSecond),
                alignment: .center
            )
            Spacer(minLength: 8)
            metric(
                "Decode speed",
                PlayaFormatting.rate(latest.decodeTokensPerSecond),
                alignment: .trailing
            )
        }
    }
...
private struct SessionStatsLoadingMenuView: View {
    ...
    var body: some View {
        ...
        .accessibilityElement(children: .contain)
        .accessibilityLabel(section == .header
            ? "Playa Server is loading \(modelName)"
    ...
    private var header: some View {
        HStack(alignment: .top) {
            ...
            VStack(alignment: .trailing, spacing: 1) {
                ...
                Text(PlayaFormatting.truncateModelName(modelName, maxLength: 20))
                    .font(.subheadline)
                    .foregroundStyle(secondaryTextColor)
                    .lineLimit(1)
            }
