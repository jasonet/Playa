...
struct IntegrationProfileManager {
    ...
    func configurationURL(for tool: IntegrationTool) -> URL {
        ...
        case .codex:
            return home.appendingPathComponent(".codex/playa.config.toml")
        case .claudeCode:
            return integrationsSupportURL.appendingPathComponent("claude-settings.json")
        case .hermes:
            return home.appendingPathComponent(".hermes/profiles/playa/config.yaml")
        case .openCode:
    ...
    private func codexProfile(selectedModelID: String) -> String {
        """
        # Managed by Playa. Other Codex profiles are not modified.
    ...
    private func configureHermes(selectedModelID: String, models: [IntegrationModelDescriptor]) throws {
        ...
        let yaml = """
        # Managed by Playa in an isolated Hermes profile.
        model:
        ...
        if !fileManager.fileExists(atPath: profileURL.path) {
            try writeText("name: playa\ndescription: Local models from Playa\n", to: profileURL)
        }
    }
    ...
    private func updatingCodexUserConfiguration(
        ...
        func appendMissingRootValues() {
            ...
            output.append("# Model selection managed by Playa.")
            if !wroteModel {
        ...
        for line in lines {
            ...
            if !reachedTable, let key = tomlAssignmentKey(line) {
                if key == "model" {
                    output.append("model = \(tomlString(selectedModelID)) # Managed by Playa")
                    wroteModel = true
                    continue
                }
                if key == "model_provider" {
                    output.append("model_provider = \(tomlString(Self.providerID)) # Managed by Playa")
                    wroteProvider = true
                    continue
                }
            }
        ...
        output.append("")
        output.append("# Provider managed by Playa.")
        output.append("[model_providers.\(tomlString(Self.providerID))]")
        output.append("name = \(tomlString("Playa"))")
