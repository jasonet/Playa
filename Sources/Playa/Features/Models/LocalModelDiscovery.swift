...
enum LocalModelFormat: String, CaseIterable, Hashable, Sendable {
    case mlx
    case gguf
...
enum LocalModelDiscovery {
    ...
    static func expandedPath(_ path: String) -> String {
        ...
        let effectivePath = trimmed.isEmpty ? PlayaSettings.defaultModelSearchPath : trimmed
    ...
    private static func scanSynchronously(path: String) throws -> [LocalModel] {
        ...
        let models = repoURLs.compactMap { repoURL -> LocalModel? in
            ...
            return LocalModel(
                ...
                format: .mlx,
                tags: extractTags(from: repoID.split(separator: "/").last.map(String.init) ?? repoID, format: .mlx)
            )
        }
    ...
    static func scanLmStudioModels(path: String) async throws -> [LocalModel] {
        let expandedPath = NSString(string: path).expandingTildeInPath
        return try await Task.detached {
            try scanLmStudioModelsSync(path: expandedPath)
        }.value
    }

    private static func scanLmStudioModelsSync(path: String) throws -> [LocalModel] {
        let fileManager = FileManager.default
        let rootURL = URL(fileURLWithPath: path, isDirectory: true)
        var isDirectory: ObjCBool = false
        ...
        for publisherURL in publisherURLs where isDirectoryURL(publisherURL, fileManager: fileManager) {
            let publisherName = publisherURL.lastPathComponent
            ...
            for modelURL in modelURLs where isDirectoryURL(modelURL, fileManager: fileManager) {
                ...
                let repoID = "\(publisherName)/\(modelName)"

                let hasSafetensors = containsSafetensors(in: modelURL, fileManager: fileManager)
                let hasGGUF = containsGGUF(in: modelURL, fileManager: fileManager)

                guard hasSafetensors || hasGGUF else { continue }

                let format: LocalModelFormat = hasSafetensors ? .mlx : .gguf
                let modifiedAt = (try? modelURL.resourceValues(forKeys: [.contentModificationDateKey]))?.contentModificationDate

                let sizeBytes: Int64?
                if hasGGUF {
                    sizeBytes = ggufFileSize(in: modelURL, fileManager: fileManager)
                } else {
                ...
                let memoryMetadata: ModelMemoryMetadata
                if hasSafetensors {
                    memoryMetadata = modelMemoryMetadata(
                        repoID: repoID,
                        snapshotURL: modelURL,
                        fileManager: fileManager
                    )
                } else {
                    memoryMetadata = ModelMemoryMetadata(parameterCount: nil, quantizationBits: nil, quantizationGroupSize: nil)
                }
                ...
                let caps: Set<LocalModelCapability>
                if hasSafetensors {
                    caps = modelCapabilities(at: modelURL, fileManager: fileManager)
                } else {
                    caps = [.text]
                }

                let modelTags = extractTags(from: modelName, format: format)
    ...
    private static func containsSafetensors(in url: URL, fileManager: FileManager) -> Bool {
    ...
    private static func containsGGUF(in url: URL, fileManager: FileManager) -> Bool {
        guard let contents = try? fileManager.contentsOfDirectory(at: url, includingPropertiesForKeys: nil, options: [.skipsHiddenFiles]) else {
            return false
        }
        return contents.contains { $0.pathExtension.lowercased() == "gguf" }
    }

    private static func ggufFileSize(in url: URL, fileManager: FileManager) -> Int64? {
        ...
        let total: Int64 = contents
            .filter { $0.pathExtension.lowercased() == "gguf" }
            .compactMap { (try? $0.resourceValues(forKeys: [.fileSizeKey]))?.fileSize.map(Int64.init) }
            .reduce(0, +)
    ...
    static func extractTags(from modelName: String, format: LocalModelFormat) -> Set<String> {
        var tags = Set<String>()
        let lower = modelName.lowercased()

        tags.insert(format.rawValue.uppercased())

        let families: [(String, String)] = [
            ("qwen", "Qwen"), ("gemma", "Gemma"), ("llama", "Llama"),
            ("mistral", "Mistral"), ("mixtral", "Mixtral"), ("deepseek", "DeepSeek"),
            ("phi", "Phi"), ("whisper", "Whisper"), ("gpt-oss", "GPT-OSS"),
            ("flux", "FLUX"), ("olmo", "OLMo"), ("minicpm", "MiniCPM"),
            ("nemotron", "Nemotron"), ("glm", "GLM"), ("cogvlm", "CogVLM"),
            ("granite", "Granite"), ("command", "Command"), ("xunzi", "Xunzi"),
            ("openelm", "OpenELM"), ("lfm", "LFM"), ("cohere", "Cohere"),
            ("cogvideo", "CogVideo"),
        ]
        for (key, label) in families where lower.contains(key) {
            tags.insert(label)
        }

        let sizePatterns: [(String, String)] = [
            ("27b", "27B"), ("26b", "26B"), ("20b", "20B"),
            ("12b", "12B"), ("9b", "9B"), ("8b", "8B"),
            ("7b", "7B"), ("4b", "4B"), ("3b", "3B"),
            ("2b", "2B"), ("1b", "1B"), ("e4b", "E4B"),
            ("e2b", "E2B"), ("a4b", "A4B"),
        ]
        for (key, label) in sizePatterns where lower.contains(key) {
            tags.insert(label)
        }

        if format == .gguf {
            let quantPatterns: [(String, String)] = [
                ("q8_0", "Q8_0"), ("q6_k", "Q6_K"), ("q5_k_m", "Q5_K_M"),
                ("q5_k_s", "Q5_K_S"), ("q4_k_m", "Q4_K_M"), ("q4_k_s", "Q4_K_S"),
                ("q3_k_m", "Q3_K_M"), ("q3_k_s", "Q3_K_S"), ("q2_k", "Q2_K"),
                ("iq4_nl", "IQ4_NL"), ("iq4_xs", "IQ4_XS"), ("iq3_m", "IQ3_M"),
                ("iq3_xxs", "IQ3_XXS"), ("iq2_m", "IQ2_M"), ("iq2_xxs", "IQ2_XXS"),
                ("iq1_m", "IQ1_M"), ("iq1_s", "IQ1_S"),
                ("f16", "F16"), ("bf16", "BF16"),
                ("q2kp", "Q2KP"), ("q4km", "Q4KM"),
            ]
            for (key, label) in quantPatterns where lower.contains(key) {
                tags.insert(label)
            }
        }

        if format == .mlx {
            let mlxQuant: [(String, String)] = [
                ("2bit", "2-bit"), ("3bit", "3-bit"), ("4bit", "4-bit"),
                ("5bit", "5-bit"), ("6bit", "6-bit"), ("8bit", "8-bit"),
                ("mxfp8", "MXFP8"), ("fp4", "FP4"), ("bf16", "BF16"),
            ]
            for (key, label) in mlxQuant where lower.contains(key) {
                tags.insert(label)
            }
        }
        ...
        let features: [(String, String)] = [
            ...
            ("ablated", "Ablated"), ("heretic", "Heretic"),
            ("nsfw", "NSFW"), ("mtp", "MTP"),
            ("drafter", "Drafter"), ("coder", "Coder"),
            ("code", "Code"), ("vision", "Vision"),
            ("audio", "Audio"), ("video", "Video"),
            ("reasoning", "Reasoning"), ("tool", "Tools"),
            ("imatrix", "iMatrix"), ("distilled", "Distilled"),
            ("yarn", "YaRN"), ("super", "Super"),
            ("ultimate", "Ultimate"), ("neo", "NEO"),
            ("writer", "Writer"), ("erotica", "Erotica"),
            ("creative", "Creative"), ("opus", "Opus"),
            ("large", "Large"), ("turbo", "Turbo"),
            ("tiny", "Tiny"), ("small", "Small"),
            ("medium", "Medium"), ("mmproj", "MMProj"),
            ("dirty", "Dirty"),
        ]
        for (key, label) in features where lower.contains(key) {
            tags.insert(label)
        }
...
final class LocalModelLibrary: ObservableObject {
    ...
    func scan(path: String, lmStudioPath: String? = nil) {
        scanTask?.cancel()
        isScanning = true
        error = nil
        ...
        scanTask = Task { [weak self] in
            do {
                var allModels = try await LocalModelDiscovery.scan(path: path)
                if let lmStudioPath {
                    let lmStudioModels = try await LocalModelDiscovery.scanLmStudioModels(path: lmStudioPath)
                    let existingIDs = Set(allModels.map(\.repoID))
                    for model in lmStudioModels where !existingIDs.contains(model.repoID) {
