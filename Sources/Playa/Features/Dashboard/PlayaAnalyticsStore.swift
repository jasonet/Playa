...
enum PlayaAnalyticsRange: CaseIterable {
    case last24Hours
    case last7Days
    case last30Days
    case lastYear
    ...
    var granularity: PlayaAnalyticsGranularity {
        switch self {
        case .last24Hours:
            return .hour
...
enum PlayaAnalyticsGranularity: String {
    case hour
    case day
}

struct PlayaHistoricalAnalyticsSummary: Sendable {
    static let empty = PlayaHistoricalAnalyticsSummary()
    ...
    var asAllTimeStats: PlayaAllTimeStats {
        PlayaAllTimeStats(
...
struct PlayaAnalyticsBucketPoint: Identifiable, Sendable {
...
struct PlayaAnalyticsRequestEvent: Identifiable, Sendable {
...
struct PlayaAnalyticsTTFTEvent: Sendable {
    let completedAt: Date
    let milliseconds: Int64
}

final class PlayaAnalyticsStore {
    private let databaseURL: URL

    init(databaseURL: URL = PlayaAnalyticsStore.defaultDatabaseURL()) {
    ...
    func fetchSummary(
        range: PlayaAnalyticsRange = .allTime,
        modelID: String? = nil,
        granularityOverride: PlayaAnalyticsGranularity? = nil
    ) -> PlayaHistoricalAnalyticsSummary {
        ...
        return PlayaHistoricalAnalyticsSummary(
    ...
    func fetchBuckets(
        ...
        var rows: [PlayaAnalyticsBucketPoint] = []
        while (try? statement.step()) == true {
            guard let granularity = PlayaAnalyticsGranularity(rawValue: statement.string(at: 0) ?? "") else {
                continue
            }
    ...
    func fetchRecentRequestEvents(
        ...
        var rows: [PlayaAnalyticsRequestEvent] = []
        while (try? statement.step()) == true {
            rows.append(
                PlayaAnalyticsRequestEvent(
    ...
    func fetchTTFTEvents(
        ...
        var rows: [PlayaAnalyticsTTFTEvent] = []
        while (try? statement.step()) == true {
            rows.append(
                PlayaAnalyticsTTFTEvent(
