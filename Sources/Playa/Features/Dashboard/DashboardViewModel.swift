...
final class DashboardViewModel: ObservableObject {
    struct ModelOption: Identifiable, Hashable, Sendable {
        ...
        var displayTitle: String {
            title == "All" ? title : PlayaFormatting.truncateModelName(title, maxLength: 52)
        }
    }
    ...
    enum RangeOption: String, CaseIterable, Identifiable, Sendable {
        ...
        var analyticsRange: PlayaAnalyticsRange {
            switch self {
            case .last24Hours:
                .last24Hours
            case .last7Days:
        ...
        var defaultGranularity: PlayaAnalyticsGranularity {
    ...
    @Published private(set) var historicalSummary = PlayaHistoricalAnalyticsSummary.empty
    ...
    @Published private(set) var recentRequestEvents: [PlayaAnalyticsRequestEvent] = []
    ...
    init(analyticsDatabaseURL: URL = PlayaAnalyticsStore.defaultDatabaseURL()) {
    ...
    func reloadHistorical() {
        ...
        let task = Task.detached(priority: .userInitiated) {
            let store = PlayaAnalyticsStore(databaseURL: databaseURL)
...
private extension DashboardViewModel {
    ...
    struct DashboardSnapshot: Sendable {
        let summary: PlayaHistoricalAnalyticsSummary
        ...
        let recentRequestEvents: [PlayaAnalyticsRequestEvent]
    }

    nonisolated static func activityPoints(
        from rawBuckets: [PlayaAnalyticsBucketPoint]
    ) -> [ActivityPoint] {
    ...
    nonisolated static func modelPerformance(
        from buckets: [PlayaAnalyticsBucketPoint]
    ) -> [ModelPerformance] {
    ...
    nonisolated static func bucketPoints(
        from rawBuckets: [PlayaAnalyticsBucketPoint],
        ttftEvents: [PlayaAnalyticsTTFTEvent] = [],
        range: RangeOption,
    ...
    nonisolated static func displayGranularity(
        ...
        store: PlayaAnalyticsStore,
        modelID: String?,
        calendar: Calendar = .current
    ) -> PlayaAnalyticsGranularity {
