...
struct StatsView: View {
    ...
    private var sessionCards: [SessionCardValue] {
        ...
        return [
            ...
            SessionCardValue(
                title: "Server uptime",
                value: PlayaFormatting.duration(metrics?.summary.uptimeSeconds),
                help: nil
            ),
        ]
    }
    ...
    private func makeSessionCard(title: String, rawCount: Int?) -> SessionCardValue {
        ...
        let formatted = PlayaFormatting.compactCount(rawCount)
...
private struct DashboardContentView: View, Equatable {
    ...
    private var successRateLabel: String {
        ...
        return PlayaFormatting.percent(
    ...
    private var requestDetail: String {
        ...
        let ttft = PlayaFormatting.milliseconds(
    ...
    private func compact(_ value: Int) -> String {
        PlayaFormatting.compactCount(value).display
    }
...
private struct TokenUsagePanel: View {
    ...
    private var granularity: PlayaAnalyticsGranularity {
    ...
    private func yAxisLabel(for value: Double) -> String {
        ...
        case .tokens, .requests:
            PlayaFormatting.compactCount(Int(value.rounded())).display
        case .successRate:
            PlayaFormatting.percent(value)
        case .decodeSpeed:
            value == 0 ? "0 tok/s" : PlayaFormatting.rate(value)
        }
    }
...
private struct SuccessRateModelComparisonChart: View {
        ...
        ScrollView(.vertical) {
            ...
            .chartXAxis {
                AxisMarks(values: [0.0, 0.25, 0.5, 0.75, 1.0]) { value in
                    ...
                    AxisTick().foregroundStyle(DashboardPalette.axisTick)
                    if let raw = value.as(Double.self) {
                        AxisValueLabel(PlayaFormatting.percent(raw))
                            .font(.caption2)
                            .foregroundStyle(DashboardPalette.axisText)
                    }
                }
    ...
    private func requestVolumeLabel(for logarithmicValue: Double) -> String {
        ...
        return PlayaFormatting.compactCount(requests).display
    }
}
...
private struct SuccessRateModelSummaryTooltip: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 14) {
                metric("Success", PlayaFormatting.percent(summary.successRate))
                metric("Requests", PlayaFormatting.integer(summary.totalRequests))
                metric("Failed", PlayaFormatting.integer(summary.requestsFailed))
            }
        }
        .padding(10)
...
private struct SuccessRateHealthChart: View {
            ...
            .chartYAxis {
                if showsAllModels {
                    AxisMarks(position: .leading) { value in
                        ...
                        if let modelID = value.as(String.self) {
                            AxisValueLabel {
                                Text(PlayaFormatting.truncateModelName(modelID, maxLength: 22))
                                    .font(.caption2)
                                    .foregroundStyle(DashboardPalette.axisText)
                            }
                        }
    ...
    private var overallRateLabel: String {
        overallRate.map(PlayaFormatting.percent) ?? "--"
    }
...
private struct SuccessRateHealthTooltip: View {
    ...
    let granularity: PlayaAnalyticsGranularity
    let showsModel: Bool
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            metricRow(
                "Requests",
                value: PlayaFormatting.integer(segment.totalRequests),
                color: DashboardPalette.indigo
            )
            metricRow(
                "Failed",
                value: PlayaFormatting.integer(segment.requestsFailed),
                color: DashboardPalette.negative
            )
        }
        .padding(11)
    ...
    private var successRateLabel: String {
        segment.successRate.map(PlayaFormatting.percent) ?? "--"
    }
        ...
        VStack(alignment: .leading, spacing: 9) {
                    ...
                    ForEach(points) { point in
                        HStack(spacing: 10) {
                            ...
                            Text(point.modelID)
                                ...
                                .truncationMode(.middle)
                            Spacer(minLength: 12)
                            Text(PlayaFormatting.integer(point.totalTokens))
                                .fontWeight(.medium)
                                .monospacedDigit()
                        }
                        .font(.caption)
            ...
            HStack {
                Text("All models")
                    .foregroundStyle(.secondary)
                Spacer(minLength: 16)
                Text(PlayaFormatting.integer(totalTokens))
                    .fontWeight(.semibold)
                    .monospacedDigit()
            }
            .font(.caption)
...
private struct ModelOverviewTooltip: View {
            ...
            if metric == .requests, let ttftMilliseconds {
                HStack(spacing: 7) {
                    ...
                    Text("Average TTFT")
                        .foregroundStyle(.secondary)
                    Spacer(minLength: 12)
                    Text(PlayaFormatting.milliseconds(ttftMilliseconds))
                        .fontWeight(.semibold)
                        .monospacedDigit()
                }
                .font(.caption)
    ...
    private func valueLabel(for point: DashboardViewModel.ModelTokenPoint) -> String {
        switch metric {
        case .tokens:
            PlayaFormatting.integer(point.totalTokens)
        case .requests:
            PlayaFormatting.integer(point.totalRequests)
        case .successRate:
            point.successRate.map(PlayaFormatting.percent) ?? "--"
        case .decodeSpeed:
            PlayaFormatting.rate(point.decodeSpeed)
        }
    }
    ...
    private var summaryLabel: String {
        switch metric {
        case .tokens:
            return PlayaFormatting.integer(points.reduce(0) { $0 + $1.totalTokens })
        case .requests:
            return PlayaFormatting.integer(points.reduce(0) { $0 + $1.totalRequests })
        case .successRate:
            guard totalRequests > 0 else { return "--" }
            let completed = points.reduce(0) { $0 + $1.requestsCompleted }
            return PlayaFormatting.percent(Double(completed) / Double(totalRequests))
        case .decodeSpeed:
            ...
            return PlayaFormatting.rate(speed)
        }
    }
        ...
        VStack(alignment: .leading, spacing: 9) {
            ...
            HStack {
                Text("Total")
                    .foregroundStyle(.secondary)
                Spacer(minLength: 22)
                Text(PlayaFormatting.integer(point.processedTokensTotal))
                    .fontWeight(.semibold)
                    .monospacedDigit()
            }
            .font(.caption)
...
private struct TokenUsageTooltipRow: View {
    ...
    var body: some View {
        HStack {
            ...
            Spacer(minLength: 22)
            Text(PlayaFormatting.integer(value))
                .fontWeight(.medium)
                .monospacedDigit()
        }
        .font(.caption)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            case .requests:
                metricRow(
                    "Completed",
                    value: PlayaFormatting.integer(point.requestsCompleted),
                    color: DashboardPalette.positive
                )
                metricRow(
                    "Failed",
                    value: PlayaFormatting.integer(point.requestsFailed),
                    color: DashboardPalette.negative
                )
                metricRow(
                    "Average TTFT",
                    value: PlayaFormatting.milliseconds(point.averageTTFTMilliseconds),
                    color: DashboardPalette.latency
                )
            case .successRate:
                metricRow(
                    "Success rate",
                    value: successRate.map(PlayaFormatting.percent) ?? "--",
                    color: DashboardPalette.positive
                )
                metricRow(
                    "Requests",
                    value: PlayaFormatting.integer(totalRequests),
                    color: DashboardPalette.indigo
                )
                metricRow(
                    "Failed",
            ...
            case .decodeSpeed:
                metricRow(
                    "Decode speed",
                    value: PlayaFormatting.rate(decodeSpeed),
                    color: DashboardPalette.orange
                )
                metricRow(
                    "Generated",
                    value: "\(PlayaFormatting.integer(point.generatedTokensTotal)) tokens",
                    color: DashboardPalette.indigo
                )
            }
        }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            HStack(alignment: .firstTextBaseline) {
                Text(total == 0 ? "--" : PlayaFormatting.percent(Double(completed) / Double(total)))
...
private struct RequestHealthTooltip: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            HStack {
                Text("Total \(PlayaFormatting.integer(total))")
                Spacer(minLength: 8)
                Text(successRate)
                    .fontWeight(.semibold)
            }
    ...
    private var successRate: String {
        ...
        return PlayaFormatting.percent(Double(point.requestsCompleted) / Double(total))
    }
...
private struct RequestHealthStat: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text(PlayaFormatting.compactCount(value).display)
...
private struct ModelPerformanceTable: View {
    ...
    private func modelRow(_ row: DashboardViewModel.ModelPerformance) -> some View {
        HStack(spacing: 18) {
            ...
            tableValue(PlayaFormatting.compactCount(row.processedTokens).display, width: 105)
            tableValue(PlayaFormatting.integer(row.totalRequests), width: 90)
            tableValue(row.successRate.map(PlayaFormatting.percent) ?? "--", width: 85)
            tableValue(PlayaFormatting.rate(row.averageDecodeTokensPerSecond), width: 105)
            tableValue(PlayaFormatting.gigabytes(fromBytes: row.peakMemoryBytes), width: 105)
        }
        .padding(.vertical, 13)
    }
...
private struct DashboardRecentRequestsTable: View {
    let requests: [PlayaAnalyticsRequestEvent]
    @State private var selectedRequest: PlayaAnalyticsRequestEvent?
...
private struct DashboardRecentRequestRow: View {
    let request: PlayaAnalyticsRequestEvent
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            LazyVGrid(
                ...
                RequestDetailMetric(title: "Prompt tokens", value: PlayaFormatting.integer(request.promptTokens))
                RequestDetailMetric(title: "Output tokens", value: PlayaFormatting.integer(request.generatedTokens))
                RequestDetailMetric(title: "Elapsed", value: "\(PlayaFormatting.seconds(fromMilliseconds: request.requestElapsedMilliseconds))s")
                RequestDetailMetric(title: "Prefill", value: "\(PlayaFormatting.decimal(request.resolvedPrefillTokensPerSecond)) tok/s")
                RequestDetailMetric(title: "Decode", value: "\(PlayaFormatting.decimal(request.resolvedDecodeTokensPerSecond)) tok/s")
                RequestDetailMetric(title: "Peak memory", value: PlayaFormatting.gigabytes(fromBytes: request.peakMemoryBytes))
            }
...
private struct DashboardRecentRequestsDataRow: View {
    ...
    private func cell(for column: DashboardRecentRequestsColumn) -> some View {
        ...
        case .mode:
            ...
            .help(modeTitle)
        case .prompt:
            Text(PlayaFormatting.integer(request.promptTokens))
        case .completion:
            Text(PlayaFormatting.integer(request.completionTokens))
        case .prefill:
            Text(PlayaFormatting.decimal(request.resolvedPrefillTokensPerSecond))
        case .decode:
            Text(PlayaFormatting.decimal(request.resolvedDecodeTokensPerSecond))
        case .request:
            Text(PlayaFormatting.decimal(request.requestTokensPerSecond))
        case .elapsed:
            Text(PlayaFormatting.seconds(fromMilliseconds: request.requestElapsedMilliseconds))
        case .peakMemory:
            Text(PlayaFormatting.gigabytes(fromBytes: request.peakMemoryBytes))
        }
    }
    ...
    private var finishTitle: String {
        ...
        if let finishReason = request.finishReason,
           !finishReason.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return PlayaFormatting.titleizedIdentifier(finishReason)
        }
...
private struct DashboardTokenChart: View {
    ...
    private var chartGranularity: PlayaAnalyticsGranularity {
        points.first?.granularity ?? fallbackGranularity
    }

    private var fallbackGranularity: PlayaAnalyticsGranularity {
        switch range {
        case .last24Hours:
            .hour
        ...
        if points.isEmpty {
            ...
            .chartYAxis {
                AxisMarks(position: .leading, values: .automatic(desiredCount: 4)) { value in
                    ...
                    if let rawValue = value.as(Double.self) {
                        AxisValueLabel(centered: false) {
                            Text(PlayaFormatting.compactCount(Int(rawValue.rounded())).display)
                                .font(.caption2)
                                .foregroundStyle(DashboardPalette.axisLabel)
                        }
                    }
...
private enum DashboardChartAxis {
    ...
    static func label(
        for date: Date,
        granularity: PlayaAnalyticsGranularity,
