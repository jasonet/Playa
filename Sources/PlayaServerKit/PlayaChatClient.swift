...
public enum PlayaChatError: Error, LocalizedError, CustomStringConvertible {
...
public final class PlayaChatClient {
    ...
    public func completeChat(_ request: MLXChatCompletionRequest) async throws -> MLXChatCompletion {
        ...
        guard (200..<300).contains(httpResponse.statusCode) else {
            throw PlayaChatError.httpStatus(httpResponse.statusCode, String(decoding: data, as: UTF8.self))
        }
    ...
    public func streamChat(
        ...
        guard (200..<300).contains(httpResponse.statusCode) else {
            throw PlayaChatError.httpStatus(httpResponse.statusCode, await readErrorBody(from: bytes))
        }
        ...
        for try await line in bytes.lines {
            ...
            guard let data = dataString.data(using: .utf8) else {
                throw PlayaChatError.malformedStreamEvent(dataString)
            }
