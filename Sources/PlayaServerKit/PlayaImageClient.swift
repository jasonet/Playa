...
public enum PlayaImageError: Error, LocalizedError, CustomStringConvertible {
...
public final class PlayaImageClient {
    ...
    private func post<Payload: Encodable>(
        ...
        var fallbackError: PlayaImageError?

        for path in paths {
            do {
                return try await post(payload, path: path)
            } catch let error as PlayaImageError where error.statusCode == 404 || error.statusCode == 405 {
                fallbackError = error
                continue
            }
        }

        throw fallbackError ?? PlayaImageError.invalidResponse
    }
    ...
    private func post<Payload: Encodable>(
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw PlayaImageError.invalidResponse
        }
        guard (200..<300).contains(httpResponse.statusCode) else {
            throw PlayaImageError.httpStatus(httpResponse.statusCode, String(decoding: data, as: UTF8.self))
        }
